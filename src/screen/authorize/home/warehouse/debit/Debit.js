...
import {getDebit} from './redux/DebitSlice';
import {getBillAPI} from './redux/DebitApi';

export function Debit(props) {
  const dispatch = useDispatch();
  const listData = useSelector(getDebit);
  ...
  const onViewOrder = idEmployee => {
    props.navigation.navigate(APP_SCREEN.HOME.EMPLOYEE.ORDER, idEmployee);
  };
    ...
    <View style={{display: 'flex', flexDirection: 'row', width: '100%'}}>
      ...
      <View style={[styles.wrapHeader1]}>
        <Text style={[styles.textHeader, {marginRight: 3}]}>
          Tổng số đơn nợ: {listData.length}
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        ...
        title={translate('ctp:bill:title')}
      />
      <Divider />
      <View
        style={{
