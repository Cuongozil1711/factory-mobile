...
import {apiCheckImport, postImportApi} from '../../redux/ImportWareHouseApi';
import RNBeep from 'react-native-a-beep';

export function ImportWareHouseQrCode({navigation, route}) {
  ...
  const onSuccessQrCode = (res, payload) => {
    if (!res) {
      //console.log(payload);
      hideLoading();
      setDetails({
      ...
      hideLoading();
      setQrCode(null);
      Alert.alert('Xin chào thủ kho', 'Mã sản phẩm đã có trong kho', [
        {
          text: 'Đồng ý',
          onPress: () => {
            hideLoading();
  ...
  const onGetDataSucceeded = response => {
    try {
      if (response) {
        ...
        dispatch(apiCheckImport([response], onSuccessQrCode));
      }
    } catch (ex) {
      navigation.goBack();
    }
      ...
      <View
        ...
        <View style={[styles.wrapHeader]}>
          <CustomText style={{color: '#FFF'}}>
            {importListDataWareHouseDto?.name}
          </CustomText>
          <CustomText style={{color: '#FFF'}}>
            Số lượng: {importListDataWareHouseDto?.dataItem.length}
          </CustomText>
        </View>
        ...
        {!viewDetail && (
          <View style={[styles.wrapHeader1]}>
            <TouchableOpacity
              ...
              <Icon color="#FFF" size={5} as={FontAwesome} name="plus-circle" />
              <CustomText style={{color: '#FFF'}}>Sinh mã tự động</CustomText>
            </TouchableOpacity>
          </View>
        )}
        {!IsFocused || viewDetail ? (
          <ItemScanner
            listItem={listItem}
            data={details}
