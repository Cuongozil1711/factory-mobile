...
const ListViewComponent = ({
  ...
  const _renderItem = ({item, index}) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => onViewItem(item?.idReceiptImport)}
        style={{
          ...
          <HStack space={[2, 3]} justifyContent="space-between">
            <VStack>
              <CustomText
                ...
                bold>
                {item?.receiptImportWareHouse?.code}
              </CustomText>
              <CustomText
                color="coolGray.600"
                style={{fontSize: 15, color: '#6e6e6e'}}>
                {item?.receiptImportWareHouse?.name}
              </CustomText>
              <VStack
                style={{
                  display: 'flex',
            ...
            <VStack
              ...
              <VStack>
                <CustomText
                  ...
                  style={{
                    color: item?.deleteFlg === 1 ? '#6e6e6e' : 'rgb(255,98,89)',
                    fontSize: 15,
                  }}
                  alignSelf="flex-end">
   ...
   * @param number index
   */
  const _keyExtractor = item => item.idReceiptImport.toString();
